...
function JobList() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await axios.get('http://localhost:7070/details/getDetails');
      ...
      <div className="jobs-grid">
        {jobs.map((job, index) => (
          <div className="job-card" key={job._id || index}>
            {job.logo && <img src={job.logo} alt={job.company} className="logo" />}
            <div className="posted">{job.posted || 'N/A'}</div>
            ...
            <p>{job.description || "• A user-friendly interface lets you browse stunning photos and videos. Filter destinations based on interests and travel style, and create personalized recommendations."}</p>
